...
function Kmis() {
  ...
  const loadKpis = async (year = selectedYear) => {
    try {
      ...
      const topLevel = data.filter((kpi) => kpi.parent_kpi_id === null || kpi.parent_kpi_id === undefined);
      const filtered = year ? topLevel.filter((kpi) => kpi.fin_year === year) : topLevel;
      setKmis(filtered);
      setError('');
    } catch (err) {
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                ...
                <input
                  type="text"
                  value={getCategoryName(formData.category_id) || 'Not set'}
                  readOnly
                />
              </div>
              <div className="form-group">
