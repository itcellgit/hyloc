-- Migration: Add target formula support to kpi_values table
-- Allows Option 3 computation: manual actual value, computed target using formula

-- Add target_formula, target_source_kpi_value_ids, and computation_type columns if they don't exist
BEGIN;

ALTER TABLE kpi_values
ADD COLUMN IF NOT EXISTS computation_type VARCHAR(50),
ADD COLUMN IF NOT EXISTS target_formula TEXT,
ADD COLUMN IF NOT EXISTS target_source_kpi_value_ids INTEGER[];

-- Add meaningful comments to document the columns
