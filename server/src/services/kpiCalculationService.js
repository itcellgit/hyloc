...
export class KPICalculationService {
   ...
   * @param {string} valueType - 'actual' or 'target'
  ...
  static async computeCumulativeSumForSource(sourceKpiValueId, month, year, valueType = 'actual') {
    ...
    // Fiscal year starts in April, so April 2024 - March 2025 is FY 2024
    let query;
    let params;
  ...
  static async calculateKPIValue(kpiValueId, month, year, empId, valueType = 'actual') {
    try {
      ...
      // Check if all dependencies are present - if not, do not compute to avoid misleading values
      if (missingDependencies.length > 0) {
        console.warn(`[COMPUTE] Cannot compute ${kpiValue.data} for ${month}/${year}: Missing dependencies: ${missingDependencies.join(', ')}`);
        throw new Error(`Cannot compute: Missing required dependency values (${missingDependencies.join(', ')})`);
      }
      ...
      if (cumsumMatches.length > 0) {
        console.log(`[CUMSUM] Processing ${cumsumMatches.length} CUMSUM expressions for ${valueType} in month ${month}/${year}`);
        processedFormula = await (async () => {
          ...
          for (const m of cumsumMatches) {
            ...
            if (!seen.has(sourceId)) {
              ...
              console.log(`[CUMSUM] Computed cumulative ${valueType} for KPI Value ${sourceId}: ${cumVal}`);
          ...
          console.log(`[CUMSUM] Final formula after replacement: ${expr}`);
          return expr;
        })();
      }
      ...
      // console.log(`Calculated ${kpiValue.data} ${valueType}: ${result} (month: ${month}, year: ${year})`);
      return result;
    } catch (error) {
      console.error(`Error calculating KPI ${valueType} value:`, error);
      throw error;
    }
  }
    ...
    try {
      ...
      for (const kpi of computedKPIs) {
        try {
          ...
          // console.log(`Calculated ${kpi.data} actual = ${value}`);
          ...
          if (kpi.target_required) {
            try {
              ...
              // console.log(`Calculated ${kpi.data} target = ${targetToSave}`);
            } catch (error) {
